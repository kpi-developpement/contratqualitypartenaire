  ...
  return (
    <div className="space-y-8">
      ...
      {globalData && (
        ...
        >
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-br from-blue-500/20 via-indigo-500/10 to-emerald-500/20 rounded-full blur-[80px] -z-10 transform translate-x-1/3 -translate-y-1/3 group-hover:scale-125 group-hover:rotate-12 transition-transform duration-1000 ease-out" />
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
            ...
            <div className="text-left md:text-right w-full md:w-auto p-5 rounded-3xl bg-white/60 border border-white shadow-sm backdrop-blur-md group-hover:bg-white/80 transition-colors">
              <div className={cn("text-5xl md:text-6xl font-black tracking-tighter drop-shadow-sm", globalData.total >= 0 ? "text-emerald-500" : "text-red-500")}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-10 pt-8 border-t border-slate-200/60 relative z-10">
            <div className="flex items-center justify-between p-5 rounded-2xl bg-white/50 border border-slate-200/60 shadow-sm group-hover:bg-blue-50/50 group-hover:border-blue-100 transition-all duration-500">
              ...
              <span className={cn("text-2xl font-black", globalData.racc >= 0 ? "text-emerald-600" : "text-red-600")}>{(globalData.racc * 100).toFixed(2)}%</span>
            </div>
            <div className="flex items-center justify-between p-5 rounded-2xl bg-white/50 border border-slate-200/60 shadow-sm group-hover:bg-purple-50/50 group-hover:border-purple-100 transition-all duration-500">
              ...
              <span className={cn("text-2xl font-black", globalData.sav >= 0 ? "text-emerald-600" : "text-red-600")}>{(globalData.sav * 100).toFixed(2)}%</span>
            </div>
          </div>
        </motion.div>
      )}
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-2.5 bg-white/90 rounded-2xl border border-slate-200 shadow-sm sticky top-[88px] z-30 backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto overflow-visible pb-2 lg:pb-0">
          ...
          <div className="relative z-40">
            <button onClick={() => setIsSortOpen(!isSortOpen)} className="flex items-center gap-2 bg-white px-4 py-3 rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all text-sm font-bold text-slate-700 min-w-[180px] justify-between">
            ...
            <AnimatePresence>
              {isSortOpen && (
                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 top-full mt-2 w-56 bg-white border border-slate-200 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] rounded-xl overflow-hidden flex flex-col">
                  {[ { val: 'bonus-desc', label: 'Plus Performant (+)' }, { val: 'bonus-asc', label: 'Moins Performant (-)' }, { val: 'alpha', label: 'Ordre Alphabétique' } ].map(opt => (
                    <button key={opt.val} onClick={() => { setSortBy(opt.val as any); setIsSortOpen(false); }} className="w-full flex items-center justify-between px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                      {opt.label} {sortBy === opt.val && <Check size={16} className="text-blue-600" />}
                    </button>
                  ))}
                </motion.div>
              )}
          ...
          <div className="flex items-center gap-1 bg-slate-50 p-1.5 rounded-xl border border-slate-200/60 shrink-0">
            ...
            {(['ALL', 'POSITIVE', 'NEGATIVE'] as const).map((f) => (
              <button key={f} onClick={() => setFilterBy(f)} className={cn("px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5", filterBy === f ? "bg-white shadow-sm border border-slate-200 text-slate-800" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50")}>
                {f === 'ALL' && <LayoutGrid size={14} />}
                {f === 'POSITIVE' && <TrendingUp size={14} className="text-emerald-500" />}
                {f === 'NEGATIVE' && <TrendingDown size={14} className="text-red-500" />}
          ...
          {partnersList.length > 0 ? (
            partnersList.map(([partner, sums]) => (
              <motion.div
                ...
                whileHover={{ 
                  scale: 1.03, y: -8, 
                  boxShadow: sums.total >= 0 ? "0 25px 50px -12px rgba(16, 185, 129, 0.25)" : "0 25px 50px -12px rgba(239, 68, 68, 0.25)",
                  borderColor: sums.total >= 0 ? "rgba(16, 185, 129, 0.4)" : "rgba(239, 68, 68, 0.4)"
                }}
              ...
              >
                <div className={cn("absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-12 blur-[40px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0", sums.total >= 0 ? "bg-emerald-500" : "bg-red-500")} />
                ...
                <div className="flex justify-between items-start pl-2 relative z-10">
                  ...
                  <div className={cn("w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 transition-colors shadow-sm", sums.total >= 0 ? "group-hover:bg-emerald-500 group-hover:text-white" : "group-hover:bg-red-500 group-hover:text-white")}>
                ...
                <div className="mt-6 pl-2 relative z-10">
                  ...
                  <div className={cn("text-4xl font-black tracking-tighter drop-shadow-sm transition-colors duration-300", sums.total >= 0 ? "text-emerald-500 group-hover:text-emerald-600" : "text-red-500 group-hover:text-red-600")}>
                ...
                <div className="flex gap-2 mt-6 pt-4 border-t border-slate-100/80 pl-2 relative z-10">
                  <div className="flex-1 bg-white rounded-xl p-2.5 text-center border border-slate-200/60 shadow-sm group-hover:border-slate-300 transition-colors">
                    ...
                    <div className={cn("text-[15px] font-black mt-0.5", sums.racc >= 0 ? "text-emerald-600" : "text-red-600")}>{(sums.racc * 100).toFixed(2)}%</div>
                  </div>
                  <div className="flex-1 bg-white rounded-xl p-2.5 text-center border border-slate-200/60 shadow-sm group-hover:border-slate-300 transition-colors">
                    <div className="text-[9px] font-black text-slate-400 uppercase tracking-wider">SAV</div>
                    <div className={cn("text-[15px] font-black mt-0.5", sums.sav >= 0 ? "text-emerald-600" : "text-red-600")}>{(sums.sav * 100).toFixed(2)}%</div>
                  </div>
                </div>
              </motion.div>
            ))
