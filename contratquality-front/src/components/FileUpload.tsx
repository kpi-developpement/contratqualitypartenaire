...
export default function FileUpload({ title, description, icon, uploadAction, onUploadSuccess, onUploadError }: FileUploadProps) {
  ...
  const handleFileChange = (file: File | null) => {
    if (file) { setSelectedFile(file); onUploadError(""); }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      onUploadError(error.message || "Erreur d'injection");
    } finally {
    ...
    <div className="w-full h-full">
      <div
        className={cn(
          ...
          !selectedFile ? "bg-white border-slate-200/70 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-blue-400 cursor-pointer" : "bg-emerald-50/50 border-emerald-400 shadow-md",
      ...
      >
        <input type="file" ref={fileInputRef} accept=".xlsx, .xls, .csv" className="hidden" onChange={(e) => handleFileChange(e.target.files?.[0] || null)} />
        ...
        {!selectedFile ? (
          <div className="flex flex-col items-center text-center space-y-4 w-full">
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 mt-2 transform translate-y-2 group-hover:translate-y-0">
              <span className="px-5 py-2 text-[11px] font-bold text-blue-600 bg-blue-50 rounded-full border border-blue-200 shadow-sm">Choisir un fichier</span>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center text-center space-y-5 w-full animate-in zoom-in duration-300" onClick={(e) => e.stopPropagation()}>
            <div className="relative group/btn">
              ...
              <button onClick={(e) => { e.stopPropagation(); setSelectedFile(null); }} className="absolute -top-2 -right-2 p-1.5 bg-white border border-slate-200 rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500 hover:border-red-200 shadow-md transition-all scale-0 group-hover/btn:scale-100" title="Changer de fichier">
                <X size={14} strokeWidth={3} />
              </button>
            </div>
            <div className="w-full px-4">
              <p className="text-sm font-bold text-slate-800 truncate block w-full" title={selectedFile.name}>{selectedFile.name}</p>
