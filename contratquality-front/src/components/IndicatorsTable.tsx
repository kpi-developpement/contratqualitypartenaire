...
export default function IndicatorsTable({ 
  ...
  const otherRowsDef = [
    { id: "SATCLI_OK", cat: "Satcli (sur RDV OK)", zone: "" },
    { id: "SATCLI_NOK", cat: "Satcli (sur RDV NOK) 4* et 5*", zone: "" },
    { id: "PLAINTE", cat: "Taux de plainte", zone: "" },
    { id: "GEM_NOK", cat: "Transformation des GEM en TVC (OK)", zone: "" },
    { id: "TNH", cat: "Taux de RDV non honoré", zone: "" },
    { id: "CADRAGE", cat: "Conformité cadrage PTO / PBO / PM", zone: "" },
    { id: "INCOHERENCE_PTO", cat: "Incohérence PTO", zone: "" }
  ];
          ...
          {viewMode === 'bonus' && (
            ...
            >
              ...
              <tbody className="bg-white">
                ...
                {rowsDefBonus.map((row, index) => {
                  ...
                  return (
                    <tr key={`bonus-${row.id}`} className="hover:bg-slate-50/60 transition-colors border-b border-slate-100 group">
                      ...
                      <td className="py-3 px-6 text-center bg-emerald-50/20">
                ...
                {otherRowsDef.map((row) => {
                  const res = bonusResults[row.id];
                  if (!res && !targets[row.id]) return null;
                  const resultat = res?.resultat || 0;
                  const bonus = res?.bonusCalcule ?? res?.bonus_calcule ?? 0;

                  return (
                    <tr key={`bonus-${row.id}`} className="hover:bg-slate-50/60 transition-colors border-b border-slate-100 bg-[#fbfcfd]">
                ...
                <tr className="bg-emerald-100/50 border-t-2 border-emerald-200">
                  <td colSpan={7} className="py-5 px-6 text-right font-black text-emerald-900 text-lg uppercase tracking-wide">
