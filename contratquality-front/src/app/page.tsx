      ...
      if (data && data.length > 0) {
        ...
        for (const [partner, indicators] of Object.entries(bonusData || {})) {
          ...
          if (indicators && typeof indicators === 'object') {
            Object.values(indicators).forEach((ind: any) => {
              const val = Number(ind?.bonusCalcule || ind?.bonus_calcule || 0);
              if (ind?.indicatorId?.startsWith('SAV_') || ind?.indicatorId === 'AUDIT' || ind?.indicatorId === 'REE') sav += val;
              else racc += val;
            });
          }
      ...
      <div className="max-w-[1400px] mx-auto space-y-8 relative z-10 p-4 md:p-8 lg:p-12">
              ...
              <div className="bg-slate-50/80 backdrop-blur-sm border border-slate-200/80 rounded-[2rem] p-6 md:p-8 space-y-6 shadow-inner relative">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500 mt-1">Glissez-déposez vos fichiers. Le panel restera ouvert.</p>
                  </div>
                  <div className="flex p-1 bg-white rounded-xl border border-slate-200 shadow-sm mr-12 md:mr-0">
                ...
                <AnimatePresence mode="wait">
                  {uploadCategory === 'RACC' ? (
                    ...
                    <motion.div key="sav" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <FileUpload title="AUDIT" description="90e Centile" icon={<ClipboardCheck size={24} className="text-amber-500" />} uploadAction={(f) => uploadAuditFile(f, period)} onUploadSuccess={handleSuccess} onUploadError={setError} />
        ...
        {hasData && viewState === 'DETAIL' && currentReport && (
          <SlideUp delay={0.1}>
            ...
            <IndicatorsTable period={period} partner={selectedPartner} category="RACC" reportData={currentReport} targets={targets} setTargets={setTargets} />
          </SlideUp>
        )}
