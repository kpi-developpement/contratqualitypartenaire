...
export const deleteReport = async (period: string) => {
  const response = await fetch(`${API_BASE_URL}/excel/report/${period}`, { method: "DELETE" });
...
const uploadGeneric = async (endpoint: string, file: File, period: string): Promise<ReportResponse[]> => {
  ...
  const response = await fetch(`${API_BASE_URL}/excel/upload/${endpoint}`, { method: "POST", body: formData });
  if (!response.ok) {
...
export const calculateBonus = async (period: string, partner: string, configPayload: any) => {
  ...
  if (!response.ok) throw new Error("Erreur de calcul bonus");
  return response.json();
};

export const calculateAllBonuses = async (period: string, configPayload: any) => {
  const response = await fetch(`${API_BASE_URL}/bonus/calculate-all/${period}`, {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(configPayload),
  });
  if (!response.ok) throw new Error("Erreur de calcul des bonus globaux");
  return response.json();
};